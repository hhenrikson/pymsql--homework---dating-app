...
def member_check():
    ...
    if firstname == "" or firstname.isalpha() is False:
        raise ValueError("You must provide a valid first name?")
    ...
    country_location = input("Your country location: ").capitalize().isalpha()
...
if __name__ == "__main__":
    ...
    db = pymysql.connect(host="localhost", user="fisayo", password="123456", database="")
    ensure_database(db)
    db.close()

    db = pymysql.connect(host="localhost", user="fisayo", password="123456", database=DATABASE_NAME)
    ensure_table(db)

    sql = f"""
        INSERT INTO {TABLE_NAME}
        (userid, firstname, lastname, sex, age, country_location) VALUES
        (%s, %s, %s, %s, %s, %s)
    """
